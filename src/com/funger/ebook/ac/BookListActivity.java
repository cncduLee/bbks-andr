...
import com.funger.ebook.adapter.LocAdapter;
import com.funger.ebook.helper.AboutDialog;
import com.funger.ebook.helper.LocalBook;
import com.funger.ebook.helper.MarkHelper;
import com.funger.ebook.utils.PinyinListComparator;
...
public class BookListActivity extends Activity {
    ...
    private int[] menu_toolbar_image_array = { R.drawable.reading,
	    R.drawable.local, R.drawable.favourite };
	...
	listItemAdapter = new SimpleAdapter(this, listItem,// 数据源
		...
		// 动态数组与ImageItem对应的子项
		new String[] { "itemback", "ItemImage", "BookName",
			"ItemTitle", "ItemTitle1", "ItemTitle2", "ItemImage9",
			"LastImage" },
		// ImageItem的XML文件里面的一个ImageView,两个TextView ID
		new int[] { R.id.itemback, R.id.ItemImage, R.id.bookName,
			R.id.ItemTitle, R.id.ItemTitle1, R.id.ItemTitle2,
			R.id.ItemImage9, R.id.last });
	// 添加并且显示
	...
	new Thread() {
		...
		String col[] = { "path" };
		Cursor cur = db.query("localbook", col, "type=4", null, null, null,
			null);
		Cursor cur1 = db.query("localbook", col, "type=3", null, null, null,
			null);
		Integer num = cur.getCount();
		Integer num1 = cur1.getCount();
		
		System.out.println("get type = 2 count:"+num);
		System.out.println("get type = 3 count:"+num1);
		...
		for (int i = 0; i < num + num1; i++) {
		    ...
		    map.put("LastImage", "未知");
		    map.put("path", file1.getPath());
		    map.put("com", "1");
		    listItem.add(map);

		}
		System.out.println("get all !!" + listItem.size());
		//拼音排序
	...
	case 1:// 创建快捷方式
	    ...
	    addIntent.putExtra(Intent.EXTRA_SHORTCUT_NAME,
		    imap1.get("ItemTitle").toString() + ".txt");// 快捷方式的标题
	    ...
	    addIntent.putExtra(Intent.EXTRA_SHORTCUT_INTENT,
		    new Intent(Intent.ACTION_MAIN).putExtra("bbb", path1)
			    .setComponent(comp));//
	    // 快捷方式的动作
...
//	}
	...
	list.setOnItemClickListener(new OnItemClickListener() {
		...
		try {
		    ...
		    File f = new File(s);
		    if (f.length() == 0) {
			Toast.makeText(BookListActivity.this, "该文件为空文件",
	...
	toolbarGrid.setOnItemClickListener(new OnItemClickListener() {
		...
		case 1:
		    ...
		    toolbarGrid
			    .setBackgroundResource(R.drawable.channelgallery2);
		    local();
		    break;
		case 2:
		    titletext.setText("我的收藏");
		    toolbarGrid
			    .setBackgroundResource(R.drawable.channelgallery3);
		    listItem = new ArrayList<HashMap<String, Object>>();
		    adapter();
	...
	case R.id.it3:
	    // 进入关于界面
	    AboutDialog about;
	    about = new AboutDialog(BookListActivity.this,
	...
	if (bad) {
	    Toast.makeText(BookListActivity.this, "打开失败", Toast.LENGTH_SHORT)
		    .show();
	}
	// 判断是否由快捷方式进入的程序
	if (bb != null) {
